    ...
    def forward(self, feed_dict, f_sng, attribute_embedding, relation_embedding):
        ...
        for attribute, concepts in self.used_concepts["attribute"].items():
            ...
            if all_labels.dim() == all_scores.dim() - 1:
                ...
                acc_key = "acc/scene/attribute/" + attribute
                monitors[acc_key] = (
                    ((all_scores > 0).long() == all_labels.long()).float().mean()
                )
            ...
            for f in f_sng:
                cross_scores.append(
                    attribute_embedding.cross_similarity(f[1], attribute).view(-1)
                )
            ...
            monitors[acc_key] = (
                ((cross_scores > 0).long() == cross_labels.long()).float().mean()
            )
            ...
            if self.training and self.add_supervision:
                ...
                if DEBUG_SCENE_LOSS and torch.isnan(this_loss).any():
                    ...
                    embed()
                for loss_key in [
                    "loss/scene/attribute-relation/" + attribute,
                    "loss/scene",
                ]:
                    monitors[loss_key] = monitors.get(loss_key, 0) + this_loss
            ...
            if response_question_type != response_query_type:
                ...
                if self.training and self.add_supervision:
                    l = torch.tensor(
                        10,
                        dtype=torch.float,
                        device=a[0].device if isinstance(a, tuple) else a.device,
                    )
...
class ParserV1Loss(MultitaskLossBase):
    ...
    def forward(self, feed_dict, programs_pd, accuracy, loss):
        ...
        for i in range(len(feed_dict["question_raw"])):
            log_likelihood = [
                p["log_likelihood"] for p in programs_pd if i == p["scene_id"]
            ]
            if len(log_likelihood) == 0:
                continue
            log_likelihood = torch.stack(log_likelihood, dim=0)
            discounted_log_likelihood = [
                p["discounted_log_likelihood"]
                for p in programs_pd
                if i == p["scene_id"]
            ]
            ...
            if self.reward_shape is ParserV1RewardShape.LOSS:
                # reward = -loss
                rewards = 10 - torch.stack(
                    [loss[j] for j, p in enumerate(programs_pd) if i == p["scene_id"]],
                    dim=0,
                )
                ...
                rewards = torch.tensor(
                    [
                        accuracy[j]
                        for j, p in enumerate(programs_pd)
                        if i == p["scene_id"]
                    ]
                ).to(discounted_log_likelihood)
                likelihood = F.softmax(
                    log_likelihood * rewards + -1e6 * (1 - rewards), dim=-1
                )

            # \Pr[p] * reward * \nabla \log \Pr[p]
            policy_loss += (
                -(likelihood * rewards).detach() * discounted_log_likelihood
            ).sum()
